...
int Cpu::evaluatePosition(char Player)
{
    //SC_REPORT_INFO("CPU", "evaluatePosition started");

	int score=0;
	char opponent = (Player == 'O')? 'X' : '0';
	return 0;	
}

int Cpu::NegaMax(int Depth,int Alfa,int Beta,char Player) {
	...
	for(int column=1;column<=7;column++){
		...
		if(PlayNumber[column]!=0){
			write_bram(PlayNumber[column],Player);
			win=get_ip();
			if(win!=0){
				PlayOut++;
				if(Player=='O'){
